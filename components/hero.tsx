...
import { Zap, MessageSquare, TrendingUp, Bot, TableProperties, Settings } from "lucide-react"
import Image from "next/image"

export function Hero() {
  const highlights = [
    { icon: TableProperties, text: "AI reporting into Google Docs & Sheets" },
    { icon: Zap, text: "Lead intake, routing, and fewer manual handoffs" },
    { icon: Bot, text: "Workflows that save 10+ hours every week" },
  ]
      ...
      <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-4xl font-bold tracking-tight text-foreground sm:text-5xl lg:text-6xl text-balance">
              Stop doing the <span className="text-primary">same manual work</span> every week
            </h1>

            <p className="mt-6 text-lg leading-8 text-muted-foreground max-w-xl text-pretty">
              Practical AI automation for reporting, leads, and handoffs between your tools — so your
              team gets hours back and fewer mistakes.
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-4 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-2">
                ...
                Slack, Notion, Workspace &amp; CRM hooks
              </span>
              <span className="inline-flex items-center gap-2">
                <Settings className="h-4 w-4 text-primary" />
                End-to-end workflows, not one-off prompts
              </span>
            </div>
          </div>
        ...
        <div className="mt-14 pt-8 border-t border-border/50">
          <p className="text-center text-sm font-medium text-muted-foreground mb-8">
            Trusted by teams drowning in admin
          </p>
          <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
            {["Operations", "Sales", "Agencies", "Logistics", "Healthcare", "Services"].map((client) => (
              <div
                key={client}
